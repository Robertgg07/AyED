...
namespace AyED
{
	...
	public:
        ...
        {
            resultado += v_.at(i).get_val() * v_.at(i).get_inx();
        ...
        double scal_prod(const sparse_vector &v) const{
            ...
            for(int i{}; i < v_.size(); i++)
            {
                if(v_.at(i).get_val() * v_.at(i).get_inx());
            }
        return resultado;
        }

        double scal_prod(const sparse_vector &v) const  { 			int i = 0, j{0};
